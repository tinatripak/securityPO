...
app.get('/', (req, res) => {
  if (req.get(SESSION_KEY)) {
    ...
    res.redirect(`https://${process.env.DOMAIN}/authorize?client_id=${process.env.CLIENT_ID}&redirect_uri=http%3A%2F%2Flocalhost%3A3000%2FmyPage&response_type=code&response_mode=query`)
    return
  }
});
  ...
  request(options, function (error, response, body) {
    ...
    if (statusCode >= 200 && statusCode <= 299) {
      ...
      res.json({ 
        ...
        expires: information.expires_in
      });
      return
    }
